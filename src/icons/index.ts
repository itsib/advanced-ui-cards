...
export const LC_ICONS_MAP = Object.freeze(Object.keys(LC_ICONS).map(icon => Object.freeze({ name: icon })));

window.customIcons = window.customIcons || {};

window.customIcons['lc'] = {
  async getIcon(name: string): Promise<{ path: string }> {
    return { path: LC_ICONS[name.replace(/_/g, '-')] };
  },
  async getIconList(): Promise<{ name: string }[]> {
    return LC_ICONS_MAP as any;
  },
};